...
from jpasLAEs import flux_to_mag
...
def estimate_continuum(NB_flx, NB_err, N_nb=7, IGM_T_correct=True,
    ...
    for nb_idx in range(NB_flx.shape[0]):
...
def is_there_line(pm_flx, pm_err, cont_est, cont_err, ew0min,
    ...
    if not obs:
        z_nb_Arr = (w_central / w_lya - 1).reshape(-1, 1)
...
# THIS IS THE "OPTIMIZED" VERSION OF CHATGPT:
def identify_lines(line_Arr, qso_flx, cont_flx, nb_min=0, first=False):
    '''
    Returns a list of N lists with the index positions of the lines.
    ...
    for src in range(N_src):
        ...
        line_idx = np.where(line_Arr[:, src])[0]
        if len(line_idx) == 0:
            line_list.append(-1)
            line_len_list.append([-1])
            line_cont_list.append(-1)
        ...
        for i, idx in enumerate(line_idx):
            if i == 0:
                ...
                prev_idx = line_idx[i-1]
            next_idx = line_idx[i+1] if i < len(line_idx)-1 else N_fil

            this_line = np.arange(idx+1, next_idx)[qso_flx[idx+1:next_idx, src] > 0]
            if len(this_line) == 0:
                continue

            aux = -len(this_line) + nb_min + idx

            if first:  # If first=True, append continuum index to list
                this_cont_lines.append(
                    np.average(
                        this_line,
                        weights=qso_flx[this_line, src] ** 2
                    )
                )

            # Append index of the max flux of this line to the list
            this_src_lines.append(np.argmax(qso_flx[this_line, src]) + aux)
            this_src_line_flx.append(qso_flx[this_line, src].sum())

        if first:
            try:
                idx = np.argmax(np.array(this_src_line_flx) - cont_flx[this_src_lines, src])
                line_list.append(this_src_lines[idx])
                line_len_list.append(this_src_lines)
                line_cont_list.append(this_cont_lines[idx])
            except:
