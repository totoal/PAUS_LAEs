...
if __name__ == '__main__':
    ...
    # BEGIN figure
    fig, axes = plt.subplots(4, figsize=(6, 10), sharex=True)
    ...
    for field_name in  ['W3', 'W2', 'W1']:
        ...
        # panel_selection_IDs = np.array([16617799, 21282949, 4769330, 21482639])
        panel_selection_IDs = np.array([16617799, 21282949, 4769330, 17930463])
        ...
        for sel_src, refid in enumerate(sel_to_plot):
            ...
            # Plot only hand-picked selection
            if refid not in panel_selection_IDs:
                continue
            print('ok')

            ax = axes[np.where(refid == panel_selection_IDs)[0]][0]
            ...
            y_max = (data_max - data_min) * 0.35 + data_max
            ...
            ax.set_xlim(3600, 10500)
                

            # Draw CIV and CIII positions
            line_dict = {r'Ly$\alpha$': 1215.67, 'LyC': 912., 'CIV': w_CIV, 'CIII': w_CIII,
                         r'Ly$\beta$': w_lyb, 'SiIV': w_SiIV, 'MgII': w_MgII}
            for jjj, (linename, w) in enumerate(line_dict.items()):
                linename = r'\textbf{' + linename + r'}'
                ...
                if this_w_obs > 3800 and this_w_obs < 9000:
                    ax.axvline(this_w_obs,
                               ls=this_ls, color=this_color)
    ...
    fig.text(0.04, 0.5,
             r'$f_\lambda\cdot10^{17}$ [erg cm$^{-2}$ s$^{-1}$ \AA$^{-1}$]',
             va='center', rotation='vertical', fontsize=14)

    fig.subplots_adjust(hspace=0.0)

    savename = f'full_panel_selection.pdf'
    subfolder = 'paper_examples'
    fig.savefig(f'{fig_save_dir}/{subfolder}/{savename}',
                pad_inches=0.01, bbox_inches='tight', facecolor='w')
    plt.close()
